...
#' A run that early stopping *ended* is finished, whatever `epochs` says: resuming it warns, trains
#' no epoch and returns the model of the checkpoint, so that a script restarting itself converges
#' instead of training one epoch further on every attempt.
#' The same holds for a callback of your own that ends a run through `ctx$terminate`, provided it
#' sets that flag again when its state is restored, see the *State* section of [`CallbackSet`].
#'
