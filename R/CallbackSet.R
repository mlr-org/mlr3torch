...
#' The built-in callbacks therefore declare the weights of section *Callback Weights* below, spaced
#' so that a custom callback can be slotted between any two of them.
#' The same numbers are available programmatically as the `weight` column of
...
# dictionary entry; the other two are the default of CallbackSet itself and early stopping, which
# the learner builds from its `patience` parameter rather than taking from the dictionary.
...
# The 'Callback Weights' section of CallbackSet, see @eval there. Only the table is generated;
# everything explaining it is written in the roxygen block, as usual.
callback_weight_section = function() {
  entries = callback_weight_table()
  c(
    "@section Callback Weights:",
    "| weight | callback | why |",
    "| ---: | --- | --- |",
    sprintf("| `%s` | %s | %s |",
