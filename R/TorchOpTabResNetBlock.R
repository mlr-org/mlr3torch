...
TorchOpTabResNetBlock = R6Class("TorchOpBlockTabResNetBlock",
  ...
  private = list(
    .build = function(inputs, param_vals, task, y) {
      ...
      input_layer = top("linear", out_features = param_vals$d_main)$build(inputs, task, y)

      blocks = replicate(n_blocks, invoke(nn_block_resnet, .args = param_vals))
      invoke(nn_sequential, .args = c(input_layer, blocks))
    }
  )
)
...
mlr_torchops$add("tab_resnet.block", TorchOpTabResNetBlock)
