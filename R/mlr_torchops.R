...
#' @export
mlr_torchops = R6Class("DictionaryTorchOp", inherit = mlr3misc::Dictionary,
  cloneable = FALSE,
  public = list(
    metainf = new.env(parent = emptyenv()),
    add = function(key, value, metainf = NULL) {
      assert_names(key, type = "strict")
      if (grepl("\\d", key)) {
        stop("keys cannot contain digits")
      }
    ...
    get_builder = function(key) {
      obj = get0(key, envir = self$items, inherits = FALSE, ifnotfound = NULL)
      if (is.null(obj)) {
        stopf("Element with key '%s' not found!%s", key, did_you_mean(key, self$keys()))
      }
      obj$private_methods$.build
    }
  )
)$new()
