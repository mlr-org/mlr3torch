...
TorchCallback = R6Class("TorchCallback",
  ...
  public = list(
    ...
    generate = function() {
      cb = super$generate()
      if (!is.null(private$.weight)) cb$weight = private$.weight
      cb
    },
    #' @description
    #' Prints the object.
    ...
    print = function(...) {
      super$print(...)
      catn(str_indent("* Weight:", format(self$weight)))
      invisible(self)
    }
  ),
  active = list(
    #' @field weight (`numeric(1)`)\cr
    #'   The weight the generated [`CallbackSet`] will have, see its section *Ordering*.
    #'   Assigning a number overrides what the [`CallbackSet`] class declares; assigning `NULL`
    #'   hands the decision back to it.
    weight = function(rhs) {
      if (!missing(rhs)) private$.weight = assert_number(rhs, null.ok = TRUE)
      # the class declares the weight of callbacks for which it is a property of the class rather
      # than of this descriptor, such as CallbackSetCheckpoint and the lr schedulers
      private$.weight %??% class_weight(self$generator)
    }
  ),
  private = list(
    .weight = NULL,
...
#' [`as.data.table`][data.table::as.data.table], whose `weight` column is the overview of when each
#' callback is called within a stage -- compare a custom callback's weight against it, see the
#' section *Ordering* of [`CallbackSet`].
#' It is the weight the generated [`CallbackSet`] will have, which is not always `$weight` of the
#' [`TorchCallback`]: that field is only the override, and is `NULL` for a callback that leaves the
#' decision to its class.
#'
#' @family Callback
#' @family Dictionary
#' @export
...
# The weight a CallbackSet generator declares, which it may inherit. CallbackSet itself declares
# the default, so the walk only runs off the end for a generator that does not inherit from it.
class_weight = function(generator) {
  while (test_class(generator, "R6ClassGenerator")) {
