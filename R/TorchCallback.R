...
# The weight the CallbackSet generated by the TorchCallback `x` will have: the descriptor's
# override if it set one, otherwise the value its class declares, which it may inherit.
# `$weight` of the descriptor is only the override, so it is NULL for a callback that leaves the
# decision to its class -- see the *Ordering* section of CallbackSet.
callback_weight = function(x) {
  if (!is.null(x$weight)) {
    return(x$weight)
  }
