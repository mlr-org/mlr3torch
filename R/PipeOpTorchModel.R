...
PipeOpTorchModel = R6Class("PipeOpTorchModel",
  ...
  private = list(
    .train = function(inputs) {
      ...
      # `.network_hash` is deliberately left alone: the network arrives through the `ModelDescriptor`
      # rather than being part of this `PipeOp`'s configuration, and the graph it was built from
      # already identifies it. See `LearnerTorchModel`.
