...
train_loop = function(ctx, cbs, resume_path = NULL) {
  ...
  # this happens before the 'begin' stage so that callbacks see the restored state, e.g. the
...
resume_training = function(ctx, resume_path) {
  ...
  # checkpoints written by earlier versions have no state file, in which case the file suffix is
  # the best guess for the epoch (correct as long as freq_type was "epoch")
  state = if (!is.null(checkpoint$state)) readRDS(checkpoint$state)
  epochs_trained = state$epoch %??% checkpoint$suffix
  load_callback_states(ctx$callbacks, state$callbacks)

  if (epochs_trained >= ctx$total_epochs) {
    warningf(paste0("The checkpoint was trained for %i epochs, but 'epochs' is %i. No further ",
      "epochs are trained. Note that 'epochs' is the total number of epochs, including those of ",
      "the checkpoint."), epochs_trained, ctx$total_epochs)
  }
