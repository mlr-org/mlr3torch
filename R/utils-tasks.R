...
#'
#' @return A list containing elements `train` and (if `valid_split > 0`) `val`,
...
make_dl_from_task <- function(
  ...
  checkmate::assert_double(valid_split)
  checkmate::assert_integerish(batch_size)
  checkmate::assert_function(transform_train)
  ...
  checkmate::assert_logical(drop_last)

  if (is.null(transform_val) & valid_split == 0) {
    stop("No transformation for validation set provided")
  }

  # Allow for no validation split to convert test sets to dl later
  if (valid_split > 0) {
    train_ids <- sample(task$row_ids, size = ceiling(task$nrow * (1 - valid_split)))
    valid_ids <- setdiff(task$row_ids, train_ids)
  } else {
    train_ids <- task$row_ids
  }

  train_ds <- img_dataset(task$data(), row_ids = train_ids, transform = transform_train)
  train_dl <- torch::dataloader(train_ds, batch_size = batch_size, shuffle = TRUE, drop_last = drop_last)
  dls <- list(train = train_dl)

  if (valid_split > 0) {
    valid_ds <- img_dataset(task$data(), row_ids = valid_ids, transform = transform_val)
    valid_dl <- torch::dataloader(valid_ds, batch_size = batch_size, shuffle = FALSE, drop_last = drop_last)
    dls$val <- valid_dl
  }
