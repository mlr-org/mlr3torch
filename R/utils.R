...
hash_input_closure = function(x, seen = list(), depth = 0L) {
  list(hash_input(x), hash_input_captures(x, seen, depth))
}

# The free variables that a closure reads, paired with what they are bound to.
#
# Only the names that the body actually mentions are looked at, rather than everything the defining
# frame happens to hold. A function written at the top level of a script closes over the whole
# script, so hashing the frame would make `po("module", module = f)` depend on which other objects
# existed at the moment it was constructed -- two `po("module")` calls on the same function, one
# line apart, would disagree.
#
# A name that resolves in an environment that has one -- a namespace, a package environment, the
# global environment -- is recorded by that name alone: those environments stand for themselves,
# walking them would be expensive, and they are not part of what the closure "is". What is left are
# the anonymous frames a closure carries with it, which is exactly what `crate()` builds.
hash_input_captures = function(fn, seen = list(), depth = 0L) {
  env = environment(fn)
  if (!is.environment(env) || depth >= 20L) {
    return(NULL)
  }
  ids = sort(codetools::findGlobals(fn))
  set_names(map(ids, function(id) {
    owner = binding_env(id, env)
    if (is.null(owner)) {
      return("<unbound>")
    }
    name = environmentName(owner)
    if (nzchar(name)) {
      return(name)
    }
    hash_input_value(get(id, envir = owner, inherits = FALSE), seen, depth + 1L)
  }), ids)
}

# The environment `id` is bound in, searching outwards from `env`, or `NULL` if it is unbound.
binding_env = function(id, env) {
...
# `seen` guards against the reference cycles that R6 objects (`self`, `private`, `super`) and
# recursive closures introduce.
hash_input_value = function(x, seen = list(), depth = 0L) {
  ...
  # only functions and environments can close a cycle, and `identical()` on everything else would
  # mean comparing whole vectors against every object seen so far
  if (is.function(x) || is.environment(x)) {
    if (some(seen, function(y) identical(y, x))) {
      return("<not descended into>")
    }
    seen = c(seen, list(x))
  }
  if (is.function(x)) {
    return(hash_input_closure(x, seen, depth + 1L))
  }
  if (is.environment(x)) {
    ...
    values = as.list(x, all.names = TRUE, sorted = TRUE)
    return(list(names(values), map(values, hash_input_value, seen = seen, depth = depth + 1L)))
  }
  if (is.list(x)) {
