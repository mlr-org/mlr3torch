...
LearnerTorchMLP = R6Class("LearnerTorchMLP",
  ...
  public = list(
    ...
    initialize = function(task_type, optimizer = NULL, loss = NULL, callbacks = list()) {
      check_activation = crate(function(x) check_class(x, "nn_module"), .parent = topenv(parent.frame()))
      check_activation_args = crate(function(x) check_list(x, names = "unique"), .parent = topenv(parent.frame()))
      check_neurons = crate(function(x) check_integerish(x, any.missing = FALSE, lower = 1), .parent = topenv(parent.frame()))
      check_shape = crate(function(x) check_shape(x, null_ok = TRUE, len = 2L), .parent = topenv(parent.frame()))
