...
% [x] DeepL
% [ ] Change appendix
% [ ] Update readme on reproducibility
% [ ] move new results into paper
% [ ] check that torchvision bug is really due to knitr and does not happen with source()
% [ ] Check that new benchmark results are comparable3
...
More recently, \pkg{JAX} introduced what can be described as a functional code transformation framework for \pkg{NumPy} \citep{ref-harris2020array} programs, with the gradient being one such transformation.
...
One of these is \pkg{torch} \citep{ref-torch2025}, which is built directly on top of \pkg{LibTorch} and works similarly to \pytorch{}.
Various packages exist that extend \torch{}. One such extension is \pkg{torchvision}, which implements different pre-trained image models, datasets, and preprocessing operations \citep{ref-r-torchvision}.
Furthermore, \pkg{luz} \citep{ref-luz2023} and \pkg{cito} \citep{ref-cito2024} are two high-level DL libraries for \rlang{} built on top of \torch{}.
...
For example, other ML models can be seamlessly replaced by DL methods, and out-of-memory data can be preprocessed almost like standard tabular data \mlrt{}.
Through this integration, \mlrttorch{} comes with \mlrt{}’s features -- such as preprocessing, benchmarking, and hyperparameter tuning \citep{ref-mlr3tuning2024} -- which is one of its key strengths.
...
\pkg{luz}, by contrast, is currently more flexible than \mlrttorch{}, as it allows users to more easily define custom training loops, e.g., to train variational autoencoders, whereas \mlrttorch{} is currently focused on supervised learning, namely regression and classification.
...
Moreover, one advantage of \mlrttorch{} over \pkg{keras} is its unified graph language that goes beyond neural networks. This, for example, allows combining neural networks with other ML models -- such as gradient boosted trees -- via stacking \citep{ref-mlr3book-chap8}.
...
Furthermore, \mlrttorch{} allows us to define neural network architectures using the graph language defined in \mlrtpipelines{}. This makes it possible to represent the entire ML pipeline, including preprocessing and data augmentation, as well as the neural network architecture, in a single graph.
The packages are also designed to be extendable. For example, the training logic can be customized via a callback mechanism, and one can inherit from predefined classes to provide custom method implementations.
...
\Cref{sec:extending} will show how to extend the package. Then, we will demonstrate the package's capabilities using some practical use cases \Cref{sec:use-cases}.
...
\noindent with \(W_{\ell}\in\mathbb{R}^{d_{\ell}\times d_{\ell-1}}\), \(b_{\ell}\in\mathbb{R}^{d_{\ell}}\) and \(\theta = \{W_1, \ldots, W_L, b_1, \ldots, b_L\}\) for \(\ell=1,\dots,L\).
...
First, the input (e.g., a text string) is split into tokens. These tokens are then converted to a vector representation through an embedding layer.
...
The ecosystem relies \pkg{R6} \citep{ref-r6chang} for most user-facing objects and hence uses reference semantics.
...
In the example below, we train a simple decision tree \code{Learner} from the \pkg{rpart} package \citep{ref-rpart2025} on the predefined \code{mtcars} example \code{Task} \citep{ref-henderson1981building}, where the goal is to predict a car's miles per gallon (mpg) based on its other features.
...
The \pkg{mlr3pipelines} extends the \mlrt{} ecosystem with support for complete learning pipelines.
This allows composing new \code{Learner}s that, e.g., integrate pre- or post-processing steps into the learning procedure.
...
To evaluate the performance of the above \code{GraphLearner}, we can \code{resample} it using a three-fold Cross-Validation \code{Resampling} and then evaluate it using RMSE.
...
We also configure several other hyperparameters, including the device, the number of epochs, the batch size, as well as the weight decay for the optimizer, and the measures we track during training.
...
These include the predict type (\code{\$predict\_type}), how to construct the validation data \newline \noindent (\code{\$validate}), whether to run the training in a separate process (\code{\$encapsulation}), and which learner to use as a (\code{\$fallback}), if the primary \code{Learner} fails.
...
As with any other \code{Learner}, learners in \pkg{mlr3torch} are annotated with standardized metadata that describe their capabilities.
...
\begin{itemize}
    ...
    \item The \code{properties} are usually special abilities of a learner, such as being able to perform feature selection, but can also be other traits. We will cover the selected properties \code{"marshal"} and \code{"validation"}, as well as \code{"internal_tuning"} below.
\end{itemize}
...
One option is to set this field to the ratio of data to use for training.
...
Although internal tuning can be considered just another hyperparameter of a learner, standardizing this property in the \mlrt{} base package allows for seamless combination with standard offline tuning.
...
Note that we neither specify the input dimension for the first layer nor the output dimension of the second layer.
...
While the module graph takes in tensors, the generating \code{Graph} operates on a \code{Task} during both the training and prediction stages.
...
When the generating \code{Graph} is trained, the network layers (\code{nn("<key>")}) instantiate a \code{PipeOpModule} and attach it to the module graph.
Auxiliary parameters, such as the input dimension of a linear layer, are inferred from the metadata that includes the output shape of the current position in the built-up \code{Graph}.
...
The actual training of the neural network happens via the \code{PipeOpTorchModel}.
This \code{PipeOp} takes in the \code{ModelDescriptor} and converts it to a \code{LearnerTorchModel} and trains it on the \code{Task} stored in the model descriptor.
Through its hyperparameters, the \code{PipeOp} also allows specifying the generic learner parameters, such as the number of epochs.
...
The \code{ParamSet} of the resulting learner contains the hyperparameters of all the individual \code{PipeOp}s that are part of its graph.
...
\begin{itemize}
    ...
    \item \code{\$.network(task, param\_vals)}, which is the network constructor and must return a \code{nn\_module} for the specified hyperparameters and input \code{Task} at the beginning of the training loop.
...
This will be done using the predefined \code{"mtcars"} example problem \citep{ref-mtcars}, where the goal is to predict the miles per gallon (mpg) of a car, from 10 numeric features.
...
\begin{CodeInput}
R> model <- po("torch_model_regr", device = "cpu", batch_size = 512)
\end{CodeInput}
...
We will tune the network's latent dimension between $20$ and $500$, repeat the layer between $1$ and $5$ times, use ReLU or sigmoid, and set the dropout ratio to in $[0.1, 0.9]$.
...
As the tuning method, we use random search via the \code{mlrtuning} \citep{ref-mlr3tuning2024}.
...
As further measures to address the imbalanced class distribution, we upsample the minority class by a factor of $4$ and use different data augmentation methods.
...
The package primarily influencing the runtime performance is therefore \torch{} and not \code{mlr3torch}.
