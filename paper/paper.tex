...
One simple example is the \code{LearnerTorchMLP}, which represents a simple multi layer perceptron (MLP) with dropout.
...
R>     loss <- nnf_mse_loss(input, target)
R>     loss <- torch_clamp(loss, max = self$max_loss)
R>     loss
R>   }
R> )
...
This object could now be passed as construction argument \code{loss} when defining a new \code{LearnerTorch}.
...
+    task = task
+  )
R> ti$result_learner_param_vals[2:7]
\end{CodeInput}
\begin{CodeOutput}
$block.n_blocks
[1] 4
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{figures/history.png}
