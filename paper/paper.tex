...
\Abstract{
    ...
    We introduce the \proglang{R} package \pkg{mlr3torch}, a comprehensive and extensible DL framework that seamlessly integrates into the \pkg{mlr3} ecosystem.
    ...
    This representation is based on the graph language defined in the \pkg{mlr3pipelines} \proglang{R} package and allows users to define the entire deep learning workflow, including preprocessing, data augmentation, and network architecture, in a single graph.
...
Historically, \pkg{TensorFlow} popularized static computation graphs, enabling optimized deployment. Meanwhile, \pkg{PyTorch} gained traction due to its dynamic graph approach, prioritizing flexibility and allowing for easier debugging.
...
Examples for this are the Hugging Face \pkg{transformers} library \citep{ref-wolf-etal-2020-transformers} the \pkg{torchvision} \citep{ref-marcel2010torchvision} and \pkg{torchaudio} \citep{ref-yang2022torchaudio} extensions for \pytorch{}.
...
In \cpp{} there is \pkg{LibTorch}, the \cpp{} backend for \pytorch{}, which can also be used as a standalone \cpp{} library.
...
The \pkg{torchvision} \rlang{} package extends this with a collection of pre-trained models, datasets, and preprocessing operations for computer vision tasks \citep{ref-r-torchvision}.
...
Other backends are supported via \pkg{keras3} \citep{ref-keras32025} and \pkg{tensorflow} \citep{ref-r-tensorflow2024}, which both wrap the corresponding \python{} libraries via \pkg{reticulate} \citep{ref-reticulate2025}.
...
For example, the \pkg{skorch} package \citep{ref-skorch} for integrating \pkg{PyTorch} into \sklearn{}, \pkg{MLJflux} \citep{ref-MLJFlux} for \flux{} and \mlj{}, and \pkg{brulee} \citep{ref-brulee2025} for \torch{} and \tidymodels{}.
...
The \pkg{mlr3} framework offers a versatile, object-oriented, and extensible framework for various ML tasks in the \proglang{R} programming language, including regression and classification.
...
% Lazy Tensor
In the \pkg{mlr3} ecosystem, the machine learning problem and dataset are represented by the \pkg{R6} class \code{Task}.
...
To support not only standard tabular features, but also generic torch tensors, \pkg{mlr3torch} defines the \code{lazy\_tensor} type, which represents a vector of tensors with possibly heterogeneous shapes.
...
By instructing the learner to use the pre-trained weights, \pkg{torchvision} will download (and optionally cache) those weights upon first use, and the torch learner will replace the output layer of the network with a new head with the correct output dimension for the task at hand.
...
\Cref{fig:optimizer-benchmark} displays absolute and \Cref{fig:optimizer-benchmark-relative} relative results.
