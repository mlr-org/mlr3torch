...
\Abstract{
    ...
    This representation is built upon the graph-language defined in the \pkg{mlr3pipelines} \proglang{R} package and allows to define the whole deep learning workflow, including preprocessing, data augmentation and network architecture, in a single graph.
    ...
    Furthermore, we demonstrate the package's capabilities through applications such as neural architecture search, fine-tuning of pretrained models, and defining architectures for multi-modal data, along with runtime benchmarks.
}
...
However, the usage of such DL frameworks it not restricted to the training of deep neural networks and can, e.g., also be used to estimate statistical models, such as L1-regularized linear regression \citep{ref-kolb2025deep}.
...
Another noteworthy library is \pkg{tinygrad} \citep{ref-tinygrad}, which strives to be a minimal, lightweight DL framework designed for simplicity and easy extensibility across multiple hardware platforms.
...
Other languages than \proglang{Python} also have DL libraries of their own.
...
In the \proglang{R} language \citep{ref-R-base} various DL packages are available, including \pkg{keras3} \citep{ref-keras32025} and \pkg{tensorflow} \citep{ref-r-tensorflow2024}, which both wrap the corresponding \pytorch{} libraries via \pkg{reticulate} \citep{ref-reticulate2025}.
With \pkg{torch} \citep{ref-torch2025}, \proglang{R} also has a native DL framework built on top of \pkg{LibTorch} which is very similar to \pytorch{}.
The \pkg{torchvision} \rlang{} package extends this with a collection of pre-trained models, datasets, and preprocessing operation for computer vision tasks \citep{ref-r-torchvision}.
...
Besides specialized DL toolkits, there also are more general purpose ML libraries, mostly focussing on tabular data.
In \python, there is \sklearn{} \citep{ref-pedregosa2011scikit-learn}, in \julia{} there is  \mlj{} \citep{ref-blaom2020mlj}, and \rlang{} has \tidymodels{} \citep{ref-kuhn2020tidymodels} and \pkg{mlr3} \citep{ref-mlr32019} with their respective predecessors \pkg{caret} \citep{ref-kuhn2021caret} and \pkg{mlr} \citep{ref-bischl2016mlr}.
From the perspective of such general ML frameworks, neural networks are just one type of learning method besides other approaches such as statistical approaches, support vector machines, or tree-based systems.
To connect DL approaches to these generic APIs, connector packages exist, such as \pkg{skorch} \citep{ref-skorch} for integrating \pkg{PyTorch} into \sklearn{}, or similarly \pkg{MLJflux} \citep{ref-MLJFlux} for \flux{} and \mlj, and \pkg{brulee} \citep{ref-brulee2025} for \torch{} and \tidymodels.
...
By being integrated into the \mlrt{} ecosystem, this also allows for easy resampling, benchmarking, or hyperparameter tuning \citep{ref-mlr3tuning2024}.
...
Furthermore, the package is designed with extensibility in mind: the training loop is customizable via a callback mechanism and it is easily possible to inherit from predefined classes for full control.
...
The impact of neural networks on fields such as computer vision, text generation, and speech recognition in the last twelve years can hardly be overstated.
...
Where $\mathcal{V} = \{v_1, \ldots, v_n\}$ is the set of vertices, corresponding to parameterized layers or operations.
Each such vertex has a set of input channels $\mathcal{I}_v$ and output channels $\mathcal{O}_v$, and $\mathcal{E} \subseteq \cup_{v \in \mathcal{V}}\; \mathcal{I}_v \times \cup_{v \in \mathcal{V}}\; \mathcal{O}_v$ is the set of directed edges, representing data flow between input and output channels.
...
\begin{equation}
\theta^* = \arg \min_{\theta \in \Theta} \mathbb{E}_{(x, y) \sim \P} \left[ \mathcal{L}(f_{\theta}(x), y) \right]
\end{equation}
...
The \pkg{mlr3} package and its associated ecosystem offer a versatile, object-oriented, and extendable framework for various ML tasks in the \proglang{R} programming language, including regression, classification and other tasks.
...
At its core, it allows users to access, train and evaluate a wide range of machine learning algorithms implemented in the \proglang{R} ecosystem through a unified interface.

The example below shows how it can be used to evaluate a simple regression tree \code{Learner} from the \pkg{rpart} package \citep{ref-rpart2025} on the predefined mtcars example \code{Task} \citep{ref-henderson1981building}, where the goal is to predict the miles per gallon (mpg) of a car based on its other features.
We train the learner on $2/3$ of the training data and make predictions on the remaining observations.
The resulting predictions are then evaluated using the root-mean-square error (RMSE) \code{Measure}.
...
A core feature of the \code{lazy\_tensor} is that it can be processed by \code{PipeOp}s.
...
Each learner in \pkg{mlr3torch} inherits from this base class and additionally defines the network architecture, the hyperparameters, supported feature types, and other metainformation, as well as how to construct the \code{torch::dataset} for the input \code{Task}.
...
While \pkg{mlr3torch} comes with predefined and ready-to-use losses, optimizers, and callbacks, adding custom versions thereof is also supported and is shown in \Cref{sec:extending}.
...
The hyperparameter space of the composed learner contains the parameter union from the specified loss, optimizer and callbacks, some parameters that are shared among all \code{LearnerTorch} classes, as well as learner-specific parameters that, e.g., parameterize the network architecture.
...
Furthermore, \torch{} learners can be configured through fields, which distinguish themselves from parameters as they have predefined semantics that are shared between all \code{mlr3::Learner}s.
...
\end{CodeOutput}
\begin{CodeInput}
R> head(mlp$model$callbacks$history)
\end{CodeInput}
\begin{CodeOutput}
TODO
\end{CodeOutput}
...
\begin{itemize}
    ...
    \item \code{predict\_types} indicate the type of predictions the learner can make. \torch{} regression learners can currently only make response predictions and classifiers can make both response and probability predictions.
    ...
    \item \code{properties} are special requirements or abilities of the learner. An example is the ability to perform feature selection or to handle observation weights. We will cover the selected properties marshal, validation and internal tuning below.
\end{itemize}
...
To save a \torch{} learner, one therefore needs to call \code{\$marshal()} before saving it, and \code{\$unmarshal()} after reading it.
...
This validation split is also taken into account by preprocessing \code{PipeOp}s, which ensures that the prediction logic of a preprocessing \code{PipeOp} is applied to the validation data during training.
...
Therefore, it is possible to construct such a module graph via another generating graph.
...
Note that the weights of the neural network that is being built up are not yet fit to the data.
...
Such an ingress operator is an entrypoint into the neural network, but also specifies how the data is loaded from the task that is passed to the respective input during training.
For example, \code{po("torch\_ingress\_categ")} will load the categorical features from the task and convert them into a label-encoded integer-valued tensors.
...
\begin{CodeInput}
R> graph <- list(path_num, path_categ) %>>% nn("merge_cat")
\end{CodeInput}
...
During training, the pipeop will attach the same network segment repeatedly to the neural network.
...
They wrap a loss generator (such as \code{torch::nn\_mse\_loss}) and an optimizer generator (e.g., \code{torch::optim\_sgd}) respectively and annotate them with metainformation, most importantly, a \code{ParamSet}.
...
Both of these objects could now be passed as construction arguments \code{loss} and \code{optimizer} when defining a new \code{LearnerTorch}.
...
This is possible via \code{LearnerTorchModule} class which has construction arguments
\begin{itemize}
...
The names of the tokens define as which argument they will be passed to the \code{$forward} method of the module.
...
We could now train this learner on any \mlrt{} classification task that has only numeric features.
...
This will be done using the predefined \code{"california housing"} example task \citep{ref-pace1997sparse}, where the goal is to predict the median house value in California.
...
The final step in the graph is the \code{PipeOpTorchModelRegr} that does takes in a \code{ModelDescriptor} and does the actual training:
...
By setting the \code{\$validate} field to \code{"test"}, we will early stop on the test set of the resampling used for the hyperparameter tuning, i.e., the validation data of the tuning process.
...
Note that we permute the first and third dimension of the tensor as \torch{} models expect the RGB channels to be the first dimension.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/multi-input.pdf}
    \caption{Validation accuracy of the ResNet-18 model during training.}
...
As further measures to address the unbalanced class distribution, we upsample the minority class by a factor of 4, use random vertical and horizontal flipping, as well as random cropping for data augmentation.
...
Finally, we change the ID of the learner, resample it on the task using five-fold cross validation, and display the resulting ROC curve using \citep{ref-mlr3viz2025}.
...
For the optimizers in \rlang{}, we use \texttt{optim\_ignite\_adamw} and \texttt{optim\_ignite\_sgd} which wrap the \libtorch{} \cpp{} implementation of the optimizers.
The results when using the \rlang{}-implementation of the optimizers in \torch{} is considerably worse.
...
In this work, we introduced \pkg{mlr3torch}, which offers an extensive DL framework in \rlang{} that integrates with the \mlrt{} ecosystem.
...
A key strength of \pkg{mlr3torch} is its extensibility and flexibility, be it in defining architectures, customizing the training, e.g., via the provided callback mechanism, or the possiblity to extend the package to new task types.
...
Our runtime benchmarks demonstrate that the framework overhead of \mlrttorch{} compared to pure \torch{} implementations is minimal, and that the right configuration yields good runtime performance compared to \pytorch{}.
