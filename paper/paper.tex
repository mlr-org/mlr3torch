...
Analogously to the the optimizer and loss, conversion of the latter to the former is possible via \code{as\_torch\_callback()}.
...
In summary, we recommend using the ignite versions of the optimizer to obtain good runtime performance and, if possible, jit-compiling the network (via \code{torch::jit\_trace} or \code{jit\_trace} parameter in \mlrttorch{}).
