...
\begin{itemize}
    ...
    \item [ ] check whether results are really different on different hardware  ??? really?
\end{itemize}
...
\begin{itemize}
    ...
    \item [ ] add paper subdirectory also to zenodo
...
\pkg{luz}, by contrast, is currently more flexible than \mlrttorch{}, and allows to define custom training loops, e.g., to train variational autoencoders, whereas \mlrttorch{} is currently restricted to regression and classification.
...
Moreover, \mlrttorch{} distinguishes itself from \pkg{keras3} by offering a unified graph language for expressing ML workflows that extend beyond neural networks, enabling, for example, straightforward integration of neural networks with other ML models via stacking \citep{ref-mlr3book-chap8}.
...
To work with this graph using a more familiar interface, we can convert it into an \code{nn\_graph} which inherits from \code{nn\_module}.
...
The generating graph for the above module graph is specified below, where \code{nn("<key>")} is short for \code{po("nn_<key>", id = "<key>")}.
...
\begin{itemize}
    ...
    \item \code{\$.dataset(task, param\_vals)}, which must create a \code{torch::dataset} for the given input task and parameter values. The dataset must return a named list, where \code{x} is a list of feature tensors, \code{y} is the target tensor, and \code{.index} contains the batch indices.
\end{itemize}
...
We could now train this learner on an \mlrt{} classification task with numeric features.
...
As a first step, we will show how to construct the classification task, the data for which we will download with the help of the \pkg{torchdatasets} \rlang{} package \citep{torchdatasets}.
...
Now, we combine the three steps into a single new \code{GraphLearner}, configure its validation data, train it on the task, and print the validation loss.
...
Finally, we change the ID of the learner, resample it on the task using five-fold cross-validation (which will respect the grouping variable \code{patient\_id}), and display the resulting ROC curve in \Cref{fig:roc-curve} using \pkg{mlr3viz} \citep{ref-mlr3viz2025, ref-ggplot2}.
...
When reproducing results on different GPUs, results may vary beyond those of the ResNet-18 model due to differences in floating-point arithmetic.
\end{appendix}
