...
Afterwards, we show how to build architectures as `mlr3pipelines::Graph`s, which makes it possible to infer shapes from the task and to tune the architecture itself.
...
Note that in the code below `po("nn_relu_1")` is equivalent to `nn("relu", id = "nn_relu_1")`.
