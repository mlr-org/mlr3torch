...
As these are construction arguments (and not part of the learner's `ParamSet`) they cannot be changed after a learner is created.
...
Below, we retrieve the predefined `"history"` callback using `t_clbk()`, which has no parameters and merely saves the training and validation history in the learner so it can be accessed afterwards.
...
```{r, fig.align='center', out.width="50%", dpi = 300, echo = FALSE}
