...
Optimizers are represented as [`DescriptorTorchOptimizer`] objects -- they wrap a `torch` `torch_optimizer_generator` object but also provide a `ParamSet`. They can be obtained from a `Dictionary` using the `t_opt` quick-access function.
...
Loss-functions work the same: [`DescriptorTorchLoss`] are obtained from a `Dictionary` using `t_loss()` and they have `ParamSet`s.
...
`po("torch_optimizer")` is used to set the `$optimizer` slot of a [`ModelDescriptor`]; it takes the desired `DescriptorTorchOptimizer` object on construction and exports its `ParamSet`.
```{r}
