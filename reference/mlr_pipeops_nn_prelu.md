...
- `num_parameters` :: `integer(1)`: Number of a to learn. Although it
  takes an int as input, there is only two values are legitimate: 1, or
  the number of channels at input. Default: 1.
...
- [`PipeOpTorchPReLU$new()`](#method-PipeOpTorchPReLU-new)
