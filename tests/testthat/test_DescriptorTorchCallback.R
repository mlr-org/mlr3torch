test_that("Basic checks", {
  ...
  tcb2 = DescriptorTorchCallback$new(Cbt2, id = "test2")
  ...
  tcb2 = DescriptorTorchCallback$new(Cbt2, packages = c("a", "b"))
  ...
  tcb41 = DescriptorTorchCallback$new(Cbt4)
  expect_identical(tcb41$param_set$ids(), "x")
  expect_class(tcb41$param_set$params$x, "ParamUty")

  ps42 = ps(x = p_int())
  tcb42 = DescriptorTorchCallback$new(Cbt4, param_set = ps42)
  ...
  tcb5 = DescriptorTorchCallback$new(Cbt5)
  expect_true(tcb5$param_set$ids() == "x")
})

test_that("Can retrieve predefined callback", {
  x = t_clbk("checkpoint")
  expect_class(x, "DescriptorTorchCallback")
  ...
  expect_list(cbs, types = "DescriptorTorchCallback", len = 2L)
...
test_that("callback_descriptor helper function works", {
  stages = formalArgs(callback_descriptor)
  stages = stages[grepl("^on_", stages)]
  expect_set_equal(stages, mlr3torch_callback_stages)

  expect_warning(callback_descriptor(id = "Custom", public = list(
    on_edn = function() NULL, on_nde = function() NULL)))

  tcb = callback_descriptor("Custom",
  ...
  expect_class(tcb, "DescriptorTorchCallback")
...
test_that("S3 converter work as expected", {
  tcb1 = t_clbk("history")
  expect_identical(tcb1, as_descriptor_torch_callback(tcb1))
  expect_true(data.table::address(tcb1$param_set) != data.table::address(as_descriptor_torch_callback(tcb1, clone = TRUE)))
  expect_identical(tcb1, as_descriptor_torch_callback("history"))

  Cbt1 = R6Class("CallbackTorchTest1")
  ps2 = ps()
  tcb2 = as_descriptor_torch_callback(Cbt1, param_set = ps2)

  expect_equal(tcb2$id, "Cbt1")
  expect_equal(tcb2$label, "Cbt1")
  tcb3 = as_descriptor_torch_callback(tcb2, clone = TRUE)
  expect_deep_clone(tcb2, tcb3)

  test = R6Class("CallbackTorchTest2", public = list(initialize = function(a) NULL))
  tcb4 = as_descriptor_torch_callback(test)
