checkpoint_learner = function(epochs = 2L, pth, ...) {
...
resume_learner = function(epochs, ..., callbacks = list(t_clbk("history"))) {
  learner = lrn("classif.mlp", epochs = epochs, batch_size = 50, neurons = 10,
...
test_that("resuming continues from the checkpointed epoch", {
  task = tsk("iris")
  pth = tempfile()
  checkpoint_learner(epochs = 2L, pth = pth)$train(task)

  learner = resume_learner(epochs = 5L, path = pth)
  learner$train(task)
...
test_that("resuming loads the checkpointed weights", {
  ...
  pth = tempfile()
  checkpoint_learner(epochs = 2L, pth = pth)$train(task)
  saved = torch_load(file.path(pth, "network2.pt"))

  # nothing left to train, so the weights must be exactly those of the checkpoint
  learner = resume_learner(epochs = 2L, path = pth)
...
test_that("the history of the previous run is continued", {
  ...
  learner = resume_learner(epochs = 4L, path = pth)
  learner$train(task)
...
test_that("callback states that cannot be restored are skipped with a warning", {
  task = tsk("iris")
  pth = tempfile()
  cb_stateful = torch_callback("Stateful",
  ...
  learner = lrn("classif.mlp", epochs = 1L, batch_size = 50, neurons = 10,
    callbacks = list(t_clbk("checkpoint", freq = 1), cb_stateful))
  learner$param_set$set_values(cb.checkpoint.path = pth)
  learner$train(task)
...
test_that("training starts from scratch when there is no checkpoint yet", {
  ...
  learner = resume_learner(epochs = 2L, path = tempfile())
  learner$train(task)
  expect_equal(learner$model$epochs, 2L)
...
test_that("checkpoints without a state file are resumed via the file suffix", {
  ...
  file.remove(list.files(pth, pattern = "^state", full.names = TRUE))

  learner = resume_learner(epochs = 3L, path = pth)
  learner$train(task)
...
test_that("the most recent complete checkpoint is used", {
  task = tsk("iris")
  pth = tempfile()
  checkpoint_learner(epochs = 3L, pth = pth)$train(task)
  # an incomplete checkpoint (e.g. the job died while writing) must not be picked
  file.remove(file.path(pth, "optimizer3.pt"))
...
test_that("path = TRUE takes the path from the checkpoint callback", {
  ...
  # the restart-the-same-script workflow: the path is only given once
  learner = resume_learner(epochs = 4L,
    callbacks = list(t_clbk("checkpoint", freq = 1), t_clbk("history")),
    cb.checkpoint.path = pth, path = TRUE)
  learner$train(task)

  expect_equal(learner$model$epochs, 4L)
  expect_true(all(file.exists(file.path(pth, paste0("network", 1:4, ".pt")))))
  expect_equal(readRDS(file.path(pth, "state4.rds"))$epoch, 4L)
  # the history of both runs is in the final model
  expect_data_table(learner$model$callbacks$history, nrows = 4L)
})

test_that("path = TRUE errors without a checkpoint callback", {
  task = tsk("iris")
  learner = resume_learner(epochs = 2L, path = TRUE)
  expect_error(learner$train(task), "no 'checkpoint' callback")

  # the assertion happens before the dataset and the network are built: this learner would also
  # fail when constructing the dataloader, but the resume error is the one that is raised
  learner = lrn("classif.mlp", epochs = 2L, batch_size = 50, neurons = 10, drop_last = TRUE,
    path = TRUE)
  expect_error(learner$train(tsk("iris")$filter(1:10)), "no 'checkpoint' callback")
})

test_that("path is checked", {
  learner = lrn("classif.mlp")
  expect_error(learner$param_set$set_values(path = 1), "path")
  expect_error(learner$param_set$set_values(path = c("a", "b")), "path")
  # NULL and TRUE and a single string are allowed
  expect_silent(learner$param_set$set_values(path = NULL))
  expect_silent(learner$param_set$set_values(path = TRUE))
  expect_silent(learner$param_set$set_values(path = tempfile()))
})
...
test_that("early stopping continues with the score and stagnation of the previous run", {
  ...
  make = function(epochs, ...) {
    lrn("classif.mlp", epochs = epochs, batch_size = 50, neurons = 10, patience = 5L,
      validate = 0.3, measures_valid = msrs("classif.acc"), ...)
  }

  learner = make(3L, callbacks = t_clbk("checkpoint", freq = 1))
  learner$param_set$set_values(cb.checkpoint.path = pth)
  learner$train(task)

  state = readRDS(file.path(pth, "state3.rds"))$callbacks$early_stopping
  expect_names(names(state), permutation.of = c("best_epochs", "best_score", "stagnation"))

  resumed = make(5L, path = pth)
  resumed$train(task)
...
test_that("stateless callbacks do not interfere with resuming", {
  ...
  expect_null(readRDS(file.path(pth, "state2.rds"))$callbacks$progress)

  resumed = lrn("classif.mlp", epochs = 3L, batch_size = 50, neurons = 10,
    callbacks = t_clbk("progress"), path = pth)
