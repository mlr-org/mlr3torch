test_that("Autotest", {
  ...
  autotest_torch_callback(cb, list(path = tempfile()))
})
...
test_that("CallbackTorchCheckpoint manual", {
  ...
  expect_set_equal(paste0("network", 1:3, ".rds"), list.files(pth0))

  learner = lrn("classif.mlp", epochs = 3, batch_size = 1, layers = 0, d_hidden = 1, callbacks = cb)
  pth1 = tempfile()
  learner$param_set$set_values(cb.checkpoint.path = pth1, cb.checkpoint.save_last = FALSE, cb.checkpoint.freq = 2)
  learner$train(task)
  expect_set_equal(paste0("network", 2, ".rds"), list.files(pth1))
  ...
  expect_set_equal(paste0("network", 2:3, ".rds"), list.files(pth2))
})
