...
path_to_output_train = os.path.join(PATH_TO_MLR3TORCH, cache_dir, "train_small")
path_to_output_test = os.path.join(PATH_TO_MLR3TORCH, cache_dir, "ISIC_2020_Test_Input_small")

tx = torchvision.transforms.Resize((128, 128))

for f in tqdm(os.listdir(path_to_melanoma_train)):
    img = torchvision.io.read_image(path_to_melanoma_train + f)
    small_img = tx(img.float())
