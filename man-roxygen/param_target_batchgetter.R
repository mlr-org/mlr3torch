...
#'   <%= if (exists("target_batchgetter_null")) target_batchgetter_null else "If `NULL` (default), it is taken from the task via [`get_target_batchgetter()`], which the built-in task types provide but a [`TaskTorch`] does not." %>
