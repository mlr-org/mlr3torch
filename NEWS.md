...
* The default optimizer is now AdamW instead of Adam.
  (This entry is incorrect and is kept for the historical record: the default has been
  `t_opt("adam")` since before v0.1.0 and still is. What changed in this release is that
  `t_opt("adam")` resolves to the faster `torch::optim_ignite_adam`.)
