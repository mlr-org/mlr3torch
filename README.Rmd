...
# Flatten the images
po_reshape = po("trafo_reshape", size = c(-1, 28 * 28))
task_reshaped = po_reshape$train(list(task))[[1L]]
...
task_reshaped = task_reshaped$filter(1:10)
```

```{r}
graph_lrn$train(task_reshaped)
```

We can also combine the preprocessing step and the network in one graph:

```{r}
graph_mlp_lrn = as_learner(po_reshape %>>% graph_mlp)
graph_mlp_lrn$train(task)
```
